...
*while -> goes until something happens, for -> does somehting to all the elements*

### Ex. B: When would you use for-loops and when would you use while-loops? Explain in your own words in 1-2 sentences
*for when i want to do something to all the elements, while, when i want to do it until something specific*
...
while(some_number < 20){
  print("I'm still below 20, so I run!")
  print(some_number)
  some_number = some_number + 0.5
}
...
fahrenheit_function <- function(fah){
  cel <- (fah - 32) * 5 / 9
  return(cel)
}

fahrenheit_function(350)
...
clothing_function <- function(temp){
  if(temp > 25){
...
sim_values <- rep(NA,n_sims)

for (i in 1:n_sims){
  sim_values <- rlnorm(n_children, log_mean, log_sd)
}
...
for (i in 1:n_sims) {
  sim_values[i] <- mean(rnorm(127, mean, sd))
  lower_bound <- sim_values[i] - 1.96 * se
  upper_bound <- sim_values[i] + 1.96 * se
  if ((lower_bound <= treatment_effect_est &
       upper_bound >= treatment_effect_est)) {
